...
export type Product = { id: number; name: string; price: number; allergens: string[]; emoji?: string; };
const yen = (n: number) => `¥${n.toLocaleString()}`;

export default function ProductCard({ product, qty, onAdd }: { product: Product; qty: number; onAdd: () => void; }) {
  const cardCtrl = useAnimation(); const plusCtrl = useAnimation(); const qtyCtrl = useAnimation();
  ...
  const onKey = (e: React.KeyboardEvent) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); handleAdd(); } };
    ...
    <motion.div animate={cardCtrl} className="flex items-stretch p-4 bg-white rounded-2xl shadow-lg overflow-hidden h-28 md:h-32">
      ...
      <div className="flex-1 min-w-0 px-4 py-1">
        ...
        <div className="flex flex-wrap gap-2 mt-2">
          {product.allergens.map((a) => <span key={a} className="text-xs md:text-sm px-2.5 py-0.5 rounded-full bg-gray-100 border border-gray-200 text-gray-700">{a}</span>)}
        </div>
      </div>
      <div className="relative w-40 md:w-52">
        ...
        <motion.button type="button" onClick={handleAdd} onKeyDown={onKey} whileTap={{ scale: 0.98 }}
          className="h-full w-full cursor-pointer select-none flex flex-col items-center justify-center outline-none rounded-none"
          aria-label={`${product.name} を追加`} title="右側をタップで追加">
